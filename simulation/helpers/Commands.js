...
var g_Commands = {
	...
	{
		let cmpGuiInterface = Engine.QueryInterface(SYSTEM_ENTITY, IID_GuiInterface);
		let notification = { "players": [player] };
		for (let key in cmd)
	...
	{
		GetAutoFormationUnitAIs(data.entities, player).forEach(cmpUnitAI => {
	...
	"train": function(player, cmd, data)
	{
		...
		// Check entity limits
		let template = Engine.QueryInterface(SYSTEM_ENTITY, IID_TemplateManager).GetTemplate(cmd.template);
		...
		for (let ent of data.entities)
		{
			if (unitCategory)
			{
				let cmpPlayerEntityLimits = QueryOwnerInterface(ent, IID_EntityLimits);
			...
			{
				let list = queue.GetAllEntities();
				if (list.indexOf(cmd.template) === -1 && cmd.promoted)
				{
					for (let promoted of cmd.promoted)
					{
			...
			if (queue && queue.GetAllEntities().indexOf(cmd.template) != -1)
				if ("metadata" in cmd)
...
function TryConstructBuilding(player, cmpPlayer, controlAllUnits, cmd)
{
	...
	let entities = FilterEntityList(cmd.entities, player, controlAllUnits);
	if (!entities.length)
		return false;

	let foundationTemplate = "foundation|" + cmd.template;

	// Tentatively create the foundation (we might find later that it's a invalid build command)
	let ent = Engine.AddEntity(foundationTemplate);
	...
	{
		let ret = cmpBuildRestrictions.CheckPlacement();
		if (!ret.success)
		{
			if (g_DebugCommands)
	...
	// Check entity limits
	let cmpEntityLimits = QueryPlayerIDInterface(player, IID_EntityLimits);
	...
	// Angen: we substract resources as we are building
	/*
...
function GetAutoFormationUnitAIs(ents, player)
{
	return GetFormationUnitAIs(ents, player);
	...
	if (formedEnts.length == 0)
		return GetMergedFormationUnitAIs(nonformedEnts, player, "special/formations/box");
	...
	if (!nonformedEnts)
		return formationUnitAIs;
	
	return GetMergedFormationUnitAIs(nonformedEnts, player, "special/formations/box").concat(formationUnitAIs);
}
...
{
	// TODO: LOCKABLE BATALION
	// so if locked, one selection would mean whole selection
	if (ents.length == 1)
	{
		warn("one ent");
...
*/
	...
	for (let ent of ents)
	{
		...
		// for the other cases.
		let nullFormation = (formationTemplate || cmpUnitAI.GetFormationTemplate()) == "special/formations/null";
	...
	if (!formationUnitAIs.length)
	{
		...
		let clusters = ClusterEntities(formation.entities, formationSeparation);
		let formationEnts = [];
		for (let cluster of clusters)
		{
			if (!formationTemplate || !CanMoveEntsIntoFormation(cluster, formationTemplate))
			{
				...
				let lastFormationTemplate = undefined;
				for (let ent of cluster)
				{
					let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
					if (cmpUnitAI)
					{
						let template = cmpUnitAI.GetFormationTemplate();
	...
	while (clusters.length > 1)
	{
		...
		for (let i = 0; i < clusters.length; ++i)
		{
			if (i == closeClusters[1] || i == closeClusters[0])
				continue;
			let dist1 = matrix[closeClusters[1]][i] || matrix[i][closeClusters[1]];
			let dist2 = matrix[closeClusters[0]][i] || matrix[i][closeClusters[0]];
...
{
	let template = Engine.QueryInterface(SYSTEM_ENTITY, IID_TemplateManager).GetTemplate(formationTemplate);
	if (!template) {
		warn("GetFormationRequirements: template does not exists for  " + formationTemplate);
		return false;
	}
