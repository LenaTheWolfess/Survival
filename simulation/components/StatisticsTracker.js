...
StatisticsTracker.prototype.Init = function()
{
	...
	for (let res of Resources.GetCodes())
	{
		...
		this.tributesSent[res] = 0;
		this.tributesReceived[res] = 0;
		this.treasuresCollected[res] = 0;
	}
...
{
	if (this.resourcesUsed[type] === undefined) {
		error("StatisticsTracker.IncreaseResourceUsedCounter: resourcesUsed " + type + " is undefined");
		this.resourcesUsed[type] = 0;
	}
	this.resourcesUsed[type] += amount;
};

StatisticsTracker.prototype.IncreaseTreasuresCollectedCounter = function(type, amount)
{
	this.treasuresCollected[type] += amount;
};
...
StatisticsTracker.prototype.IncreaseTributesSentCounter = function(amounts)
{
	for (let t in amounts)
		if (amounts[t])
			this.tributesSent[t] += amounts[t];
};

StatisticsTracker.prototype.IncreaseTributesReceivedCounter = function(amounts)
{
	for (let t in amounts)
		if (amounts[t])
			this.tributesReceived[t] += amounts[t];
};
