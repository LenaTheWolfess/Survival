...
/**
 * Try to use resource of specified type and return if success
 */
Player.prototype.UseResource = function(type, amount)
{
	...
	if (this.resourceCount[type] < +amount)
		return false;
...
Player.prototype.SubtractResourcesOrNotify = function(amounts)
{
	...
	for (let type in amounts) {
		if (this.resourceCount[type] === undefined) {
			error("Player.SubtractResourcesOrNotify resourceCount " + type + " is undefined");
			continue;
		}
		this.resourceCount[type] -= amounts[type];
	}
...
Player.prototype.TributeResource = function(player, amounts)
{
	...
	if (cmpOurStatisticsTracker)
		cmpOurStatisticsTracker.IncreaseTributesSentCounter(amounts);
	let cmpTheirStatisticsTracker = QueryPlayerIDInterface(player, IID_StatisticsTracker);
	if (cmpTheirStatisticsTracker)
		cmpTheirStatisticsTracker.IncreaseTributesReceivedCounter(amounts);
