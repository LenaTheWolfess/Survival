...
GuiInterface.prototype.GetEntityState = function(player, ent)
{
	...
	if (cmpProductionQueue)
		ret.production = {
			"entities": cmpProductionQueue.GetAllEntities(),
	...
	// Angen
	let cmpResourceRegrow = Engine.QueryInterface(ent, IID_ResourceRegrow);
	if (cmpResourceRegrow)
	{
		ret.regrowing = cmpResourceRegrow.GetPercents();
	}
	...
	let cmpConsumer = Engine.QueryInterface(ent, IID_Consumer);
	if (cmpConsumer)
		ret.consumer = {
			"interval": cmpConsumer.GetTimer(),
			"rates": cmpConsumer.GetRates(),
			"fed": cmpConsumer.GetCanBeFed(),
			"fedRes": cmpConsumer.GetFedRes(),
			"isFed": cmpConsumer.IsFed()
		};
	let cmpPayed = Engine.QueryInterface(ent, IID_Payed);
	if (cmpPayed)
		ret.payed = {
			"interval": cmpPayed.GetInterval(),
			"code": cmpPayed.GetCode(),
			"cost": cmpPayed.GetCost(),
		};
...
GuiInterface.prototype.GetAllTrainableEntities = function(player, cmd)
{
	let trainableEnts = [];
	for (let ent of cmd.entities)
	{
		...
		for (let unit of cmpProductionQueue.GetEntitiesList(cmd.group))
			if (trainableEnts.indexOf(unit) == -1)
				trainableEnts.push(unit);
	}
	return trainableEnts;
};

GuiInterface.prototype.GetAllTrainableGroups = function(player, cmd)
{
	let trainableGroups = [];
	let used = [];
	for (let ent of cmd.entities)
	{
		...
		for (let group  of cmpProductionQueue.GetGroups()) {
			if (used.indexOf(group.id) == -1) {
				used.push(group.id);
				trainableGroups.push(group);
			}
		}
	}
...
GuiInterface.prototype.GetAllBuildableEntities = function(player, cmd)
{
	...
	for (let ent of cmd.entities)
	{
		...
		for (let building of cmpBuilder.GetEntitiesList(cmd.group))
...
GuiInterface.prototype.GetAllBuildableGroups = function(player, cmd)
{
	let buildableGroups = [];
	let used = [];
	for (let ent of cmd.entities)
	{
		...
		for (let group  of cmpBuilder.GetGroups()) {
			if (used.indexOf(group.id) == -1) {
				used.push(group.id);
...
let exposedFunctions = {
	...
	"GetAllTrainableEntities": 1,
	"GetAllTrainableGroups": 1,
	"GetAllBuildableGroups": 1,
	"SetStatusBars": 1,
	"GetPlayerEntities": 1,
