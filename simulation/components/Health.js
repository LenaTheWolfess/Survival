...
Health.prototype.IsHurt = function()
{
...
{
	this.Reduce(this.hitpoints, true);
};
 ...
 */
Health.prototype.Reduce = function(amount, refunds = false)
{
	// Before changing the value, activate Fogging if necessary to hide changes
	let cmpFogging = Engine.QueryInterface(this.entity, IID_Fogging);
	if (cmpFogging)
	...
	if (amount >= this.hitpoints)
	{
		...
		if (this.hitpoints)
		{
			...
			if (cmpIdentity && cmpIdentity.HasClass("Structure")) {
				...
				if (cmpPlayer && cmpCost) {
					let costs = cmpCost.GetResourceCosts(cmpPlayer.GetPlayerID());
