...
UnitAI.prototype.UnitFsmSpec = {
	...
	"Order.FormationWalk": function(msg) {
		...
		// Angen
		this.SetAnimationVariant(msg.data.variant);
	...
	"Order.ReturnResource": function(msg) {
		...
		if (this.CheckTargetRange(this.order.data.target, IID_ResourceGatherer) && this.CanReturnResource(this.order.data.target, true))
		{
			...
			{				
				Engine.QueryInterface(this.entity, IID_ResourceGatherer).CommitResources(this.order.data.target);
				// Stop showing the carried resource animation.
				this.SetDefaultAnimationVariant();
				
				if (this.order.data.goBackTo) {
					let pos = this.order.data.goBackTo;
					if (this.MoveToPointRange(pos.x, pos.z, 0, 0)) {
						this.SetNextState("INDIVIDUAL.MOBILEDROPSITE");
						return;
					}
				}
	...
	"FORMATIONCONTROLLER": {
		...
		"IDLE": {
			"enter": function(msg) {
				let cmpFormation = Engine.QueryInterface(this.entity, IID_Formation);
				if (this.GetStance().keepCombat)
					cmpFormation.SetVariant("combat");
				else
					cmpFormation.SetVariant("");
	...
	"INDIVIDUAL": {
		...
		"IDLE": {
			"enter": function() {
				...
				if (cmpResourceDropsite && cmpResourceDropsite.IsMobile()) {
					let cmpResourceGatherer = Engine.QueryInterface(this.entity, IID_ResourceGatherer);
					if (cmpResourceGatherer) {
						let type = cmpResourceGatherer.GetMainCarryingType();
						if (type && !cmpResourceGatherer.CanCarryMore(type)) {
							this.FindAndReturnResources(type);
							return true;
						}
					}
				}
		...
		"COMBAT": {
			...
			"APPROACHING": {
				"enter": function() {
					...
					if (this.order.data.attackType == "Slaughter")
						this.SetAnimationVariant("approach_" + this.order.data.attackType);
					else
						this.SetAnimationVariant("combat");
			...
			"ATTACKING": {
				"enter": function() {
					...
					// Angen
					if (att == "slaughter" || att == "ranged" || att == "capture")
						this.SetAnimationVariant("attack_"+att);
					else
						this.SetAnimationVariant("combat");
					this.SelectAnimation(animationName);
		...
		"GATHER": {
			"APPROACHING": {
				"enter": function() {
					    ...
					    (!cmpSupply || !cmpSupply.AddGatherer(cmpOwnership.GetOwner(), this.entity)))
					{
						...
						let oldType = this.order.data.type;
						let oldTarget = this.order.data.target;
						let oldTemplate = this.order.data.template;
						...
						else
						{
							...
							else
							{
								...
								nearby = this.FindNearestDropsite(oldType.generic);
								if (nearby)
								{
			...
			"GATHERING": {
				"enter": function() {
					...
					{
						let typename = "gather_" + this.order.data.type.specific;
						//Angen
		...
		"REPAIR": {
			"APPROACHING": {
				"enter": function() {
					//Angen
					this.SetAnimationVariant("approach_build");
					this.SelectAnimation("move");
				},
			...
			"REPAIRING": {
				"enter": function() {
					...
					this.SetAnimationVariant("build");
					this.StartTimer(1000, 1000);
					return false;
				},
...
UnitAI.prototype.OnNearbyUnitAttacked = function(msg)
{
...
UnitAI.prototype.WarnNearbyUnitsOfAttack = function(attacker)
{
	let cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
	if (!cmpOwnership)
		return;
...
UnitAI.prototype.FindNearbyResource = function(filter, target)
{
	...
	let nearby = cmpRangeManager.ExecuteQuery(entity, 0, range, players, IID_ResourceSupply);
...
UnitAI.prototype.FindNearestDropsite = function(genericType)
{
	...
	let cmpResourceDropsite = Engine.QueryInterface(this.entity, IID_ResourceDropsite);
	let skipMobile = cmpResourceDropsite && cmpResourceDropsite.IsMobile();
	...
	for (let dropsite of nearbyDropsites)
	{
		...
		if (skipMobile && cmpResourceDropsite && cmpResourceDropsite.IsMobile())
			continue;
...
UnitAI.prototype.SetDefaultAnimationVariant = function()
{
	...
	if (!cmpResourceGatherer)
	{
		...
		{
			let cmpFormation = Engine.QueryInterface(this.formationController, IID_Formation);
			this.SetAnimationVariant(cmpFormation.GetVariant());
			return;
		}
	...
	if (this.IsFormationMember() && this.GetStance().keepCombat) {
		this.SetAnimationVariant("combat");
		return;
	}
	...
	{
		let cmpFormation = Engine.QueryInterface(this.formationController, IID_Formation);
		av = cmpFormation.GetVariant();
		this.SetAnimationVariant(av);
		return;
	}
...
UnitAI.prototype.CanReturnResource = function(target, checkCarriedResource)
{
	...
	if (cmpResourceDropsite.IsMobile()) {
		// Verify that if we are mobile dropsite, target is not
		let cmpMResourceDropsite = Engine.QueryInterface(this.entity, IID_ResourceDropsite);
		if (cmpMResourceDropsite && cmpMResourceDropsite.IsMobile())
			return false;
	}
...
/**
 * Special order called from mobile dropsites
*/
UnitAI.prototype.FindAndReturnResources = function(type)
{
	let nearby = this.FindNearestDropsite(type);
	if (!nearby)
		return;
	let pos = undefined;
	let cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
	if (cmpPosition)
		...
		error("pos is undefined");
	this.PushOrderFront("ReturnResource", { "target": nearby, "force": true, "goBackTo":  pos});	
}
