...
ResourceGatherer.prototype.TryInstantGather = function(target)
{
	...
	if (cmpStatisticsTracker)
		cmpStatisticsTracker.IncreaseTreasuresCollectedCounter(type.specific, status.amount);
...
ResourceGatherer.prototype.PerformGather = function(target)
{
	...
	if (cmpKnowledgeManager)
		cmpKnowledgeManager.AddPoints("Gather", type.generic, status.amount);
...
/**
 * Special function used for mobile dropsites with limited carry
*/
ResourceGatherer.prototype.Take = function(type, amount)
{
	if (!this.carrying[type])
		this.carrying[type] = 0;
	
	let cap = this.GetCapacity(type);
	
	let free = Math.max(0, cap - this.carrying[type]);
	let taken = Math.min(free, amount);
	let has = this.carrying[type];
	...
	this.carrying[type] += taken;
	
	// Lock taking and return resource
	if (this.carrying[type] >= cap) {
		let cmpResourceDropsite = Engine.QueryInterface(this.entity, IID_ResourceDropsite);
		if (!cmpResourceDropsite) {
			error("Mobile dropsite with limited carry does not have IID_ResourceDropsite");
			return taken;
		}
		cmpResourceDropsite.Lock();
		let cmpUnitAI = Engine.QueryInterface(this.entity, IID_UnitAI);
		if (cmpUnitAI)
			cmpUnitAI.FindAndReturnResources(type);
	}
...
ResourceGatherer.prototype.CommitResources = function(target)
{
	...
	let cmpResourceGatherer = Engine.QueryInterface(target, IID_ResourceGatherer);
	// Dropsite only gathers what we give him
	if (cmpResourceGatherer) {
		for (let type of types) {
			if (type in this.carrying) {
				this.carrying[type] -= cmpResourceGatherer.Take(type, this.carrying[type]);
				if (!this.carrying[type])
	...
	cmpResourceDropsite = Engine.QueryInterface(this.entity, IID_ResourceDropsite);
	if (cmpResourceDropsite)
		cmpResourceDropsite.Unlock();
	Engine.PostMessage(this.entity, MT_ResourceCarryingChanged, { "to": this.GetCarryingStatus() });
};
