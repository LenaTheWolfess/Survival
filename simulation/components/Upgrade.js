...
Upgrade.prototype.Init = function()
{
	...
	for (let choice in this.template)
	{
		...
		if (cmpIdentity)
			name = name.replace(/\{native\}/g, cmpIdentity.GetCiv());
		
		if (cmpPlayer)
			name.replace(/\{civ}/g, cmpPlayer.GetCiv());
...
Upgrade.prototype.GetUpgrades = function()
{
	...
	for (let option in this.template)
	{
		...
		let templateName = cmpIdentity ? choice.Entity.replace(/\{native\}/g, cmpIdentity.GetCiv()) : choice.Entity;

		templateName.replace(/\{civ}/g, cmpPlayer.GetCiv());
		let cost = {};
		if (choice.Cost)
...
Upgrade.prototype.GetRequiredTechnology = function(templateArg)
{
	...
	if (cmpIdentity)
		entType = entType.replace(/\{native\}/g, cmpIdentity.GetCiv());
