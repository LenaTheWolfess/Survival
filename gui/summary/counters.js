...
function updateCountersPlayer(playerState, counters, headings, idGUI, index)
{
	for (let n in counters)
	{
		...
		let gu = Engine.GetGUIObjectByName(idGUI + "[" + n + "]");
		if (!gu) {
			error(idGUI + "[" + n + "] is undefined");
			continue;
		}
		gu.caption =	formatSummaryValue(fn && fn(playerState, index, headings[+n + 1].identifier));
	}
}
...
function calculateTeamCounterDataHelper()
{
	...
	for (let i = 0; i < g_PlayerCount; ++i)
	{
		...
		if (!g_TeamHelperData[playerState.team])
		{
			...
			for (let value of ["meat", "fruit", "grain", "femaleCitizen", "worker", "enemyUnitsKilled",
		...
		for (let v in gatherSt) {
			let r = gatherSt[v];
			let add = playerState.sequences.resourcesGathered[r];
			if (add === undefined) {
				warn("resourcesGathered["+r+"] is undefined");
				continue;
			}
			summaryAddObject(g_TeamHelperData[playerState.team][r], add);
		}
...
function calculateResources(playerState, index, type)
{
	return {
		...
		"treasure":  playerState.sequences.treasuresCollected[type][index],
		"used": playerState.sequences.resourcesUsed[type][index] - playerState.sequences.resourcesSold[type][index]
	};
}
function calculateTributes(playerState, index, type)
{
	return {
		"sent":  playerState.sequences.tributesSent[type][index],
		"received": playerState.sequences.tributesReceived[type][index]
	};
}

function calculateTotalTribute(playerState, index)
{
	let totalTributeSent = 0;
	let totalTributeRec = 0;
	for (let type of g_ResourceData.GetCodes()) {
		totalTributeSent += playerState.sequences.tributesSent[type][index];
		totalTributeRec += playerState.sequences.tributesReceived[type][index];
	}
	
	return {
		"sent":  totalTributeSent,
		"received": totalTributeRec
	};
}
...
function calculateTotalResources(playerState, index)
{
	...
	for (let type of g_ResourceData.GetCodes())
	{
		...
		totalTreasure += playerState.sequences.treasuresCollected[type][index];
	}

	return { "gathered": totalGathered, "treasure": totalTreasure, "used": totalUsed };
}
