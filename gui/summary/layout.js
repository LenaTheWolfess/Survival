var getScorePanelsData = () => ({
	...
	"buildings": {
		...
		"titleHeadings": [
			{
				"caption": sprintf(translate("Buildings Statistics (%(constructed)s / %(lost)s / %(captured)s / %(destroyed)s)"),
					{
	...
	"units": {
		...
		"titleHeadings": [
			{
				"caption": sprintf(translate("Units Statistics (%(trained)s / %(lost)s / %(captured)s / %(killed)s)"),
					{
	...
	"resources": {
		...
		"titleHeadings": [
			{
				"caption": sprintf(translate("Resource Statistics (%(gathered)s / %(treasure)s / %(used)s)"),
					{
						"gathered": getColoredTypeTranslation("gathered"),
						"treasure": getColoredTypeTranslation("treasure"),
		...
		"counters": [
			{ "width": 110, "fn": calculateTotalResources, "verticalOffset": 3 },
	...
	"tribute": {
		...
		"titleHeadings": [
			{
				"caption":  sprintf(translate("Tributes (%(sent)s / %(received)s)"),
					{
		...
		"counters": [
			{ "width": 110, "fn": calculateTotalTribute, "verticalOffset": 12 },
			...g_ResourceData.GetCodes().map(code => ({
				"fn": calculateTributes,
				"verticalOffset": 12,
				"width": 100
			})),
		],
	...
	"market": {
		...
		"headings": [
			{ "identifier": "playername", "caption": translate("Player name"), "yStart": 34, "width": 200 },
			{ "identifier": "tradeIncome", "caption": translate("Trade income"), "yStart": 34, "width": 100 },
			{ "identifier": "barterEfficency", "caption": translate("Barter efficiency"), "yStart": 34, "width": 100 },
			...g_ResourceData.GetResources().map(res => {
				return {
					"identifier": res.code,
					"caption":
						// Translation: use %(resourceWithinSentence)s if needed
						sprintf(translate("%(resourceFirstWord)s"), {
		...
		"titleHeadings": [
			{
				"caption": sprintf(translate("Market Statistics (%(bought)s / %(sold)s)"),
					{
						"bought": getColoredTypeTranslation("bought"),
						"sold": getColoredTypeTranslation("sold")
					}),
				"yStart": 16,
				"width": 100 * g_ResourceData.GetCodes().length + 110
			},
...
function updateGeneralPanelCounter(counters)
{
	...
	for (let p = 0; p < g_MaxPlayers; ++p)
	{
		...
		for (let w in counters)
		{
			...
			if (!counterObject) {
				error ("valueData[" + p + "][" + w + "] is undefined");
				continue;
			}
		...
		for (let t = 0; t < g_MaxTeams; ++t)
		{
			...
			for (let w in counters)
			{
				...
				if (!counterObject) {
					error ("valueDataTeam[" + t + "][" + p + "][" + w + "] is undefined");
					continue;
				}
				counterObject.size = left + " " + counters[w].verticalOffset + " " + (left + counters[w].width) + " 100%";
				counterObject.hidden = false;
				...
				if (g_Teams[t])
				{
					...
					if (!counterTotalObject) {
						error ("valueDataTeam[" + t + "][" + w + "] is undefined");
						continue;
					}
