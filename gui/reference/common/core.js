...
function loadProductionQueue(template)
{
	...
	if (template.ProductionQueue.Groups) {
		for (let g in template.ProductionQueue.Groups) {
			if (template.ProductionQueue.Groups[g].Entities && template.ProductionQueue.Groups[g].Entities._string) {
				for (let templateName of template.ProductionQueue.Groups[g].Entities._string.split(" "))
				{
...
function loadBuildQueue(template)
{
	...
	for (let g in template.Builder.Groups) {
		let group = template.Builder.Groups[g];
		for (let build of group.Entities._string.split(" "))
		{
