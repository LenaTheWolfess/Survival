...
EntitySelection.prototype.addList = function(ents, quiet, force = false)
{
	...
	if (added.length)
	{
		...
		let owner = GetEntityState(added[0]).player;
	...
	for (let ent in this.highlighted)
		if (!highlighted[ent] && !this.selected[ent]) {
			...
			if (!entState)
				continue;
			if (entState.unitAI && entState.unitAI.formationController) {
				if (added.indexOf(entState.unitAI.formationController) != -1)
					continue;
				let fcs = GetEntityState(entState.unitAI.formationController);
				if (fcs) {
					for (let member of fcs.formation.members)
						removed.push(+member);
				}
				ent = entState.unitAI.formationController;
			}
			removed.push(+ent);
		}
...
var g_allBuildableGroups;
var g_selectedBuildingGroup;
var g_selectedTrainingGroup;
...
function onSelectionChange()
{
	...
	g_allBuildableGroups = undefined;
