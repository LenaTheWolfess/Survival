...
function updateTopPanel()
{
	...
	Engine.GetGUIObjectByName("diplomacyButton").hidden = !isPlayer;Engine.GetGUIObjectByName("tradeButton").hidden = !isPlayer ||
...
function updateFormationSelection()
{
	...
	for (let sel in selected)
	{
		...
		if (!state.unitAI || !state.formation)
			continue;
		let formation = selected[sel];
		if (used.indexOf(formation) != -1)
			continue;
		used.push(formation);

		let ents = state.formation.members;
		for (let ent of ents)
...
function updateAdditionalHighlight()
{
	...
	for (let sel in highlighted)
	{
		...
		if(!state || !state.unitAI || !state.formation)
			continue;
		let formation = highlighted[sel];
		if (used.indexOf(formation) != -1)
			continue;
		used.push(formation);
