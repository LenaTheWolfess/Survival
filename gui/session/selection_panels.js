...
g_SelectionPanels.ConstructionGroups = {
	"getMaxNumberOfItems": function()
	{
		return 5;
	},
	...
	"setupButton" : function(data) {
		data.button.onPress = function() 
		{
			g_selectedBuildingGroup = data.item.id;
			g_allBuildableEntities = undefined;
		};
...
g_SelectionPanels.Construction = {
	...
	"getItems": function()
	{
		if (!g_selectedBuildingGroup)
			return [];
		return getAllBuildableEntitiesFromSelection(g_selectedBuildingGroup);
	},
	"rowLength": 10,
	"setupButton": function(data)
	{
	...
	"setupButton": function(data)
	{
		...
		for (let tech of data.item.tech.pair ? [data.item.tech.bottom, data.item.tech.top] : [data.item.tech])
		{
			...
			for (let res in template.cost) {
				if (data.item.techCostMultiplier[res] === undefined) {
					error("techCostMultiplier for " + res + " is undefined");
					continue;
				}
...
g_SelectionPanels.TrainingGroups = {
	"getMaxNumberOfItems": function()
	{
		return 5;
	},
	...
	"setupButton" : function(data) {
		data.button.onPress = function() 
		{
			g_selectedTrainingGroup = data.item.id;
			g_allTrainableEntities = undefined;
		};
