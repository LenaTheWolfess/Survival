...
function resizeDiplomacyDialog()
{
	...
	let resTribCodesLength = g_ResourceData.GetCodes("tributable").length;
	
	let tribSize = Engine.GetGUIObjectByName("diplomacyPlayer[0]_tribute[0]").size;
	let widthOffset = resTribCodesLength * (tribSize.right - tribSize.left) / 2;	size.left -= widthOffset;
...
{
	let resTribCodes = g_ResourceData.GetCodes("tributable");
	let r = 0;
...
function resizeTradeDialog()
{
	...
	let resTradCodesLength = g_ResourceData.GetCodes("tradable").length;
	Engine.GetGUIObjectByName("tradeDialogPanelTrade").hidden = !resTradCodesLength;

	let resBarterCodesLength = g_ResourceData.GetCodes("barterable").length;
