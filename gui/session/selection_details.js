...
function displaySingle(entState)
{
	...
	let showRegrow = entState.regrowing !== undefined;
	...
	if (showHealth)
	{
		...
		captureSection.size = showResource || showRegrow ? sectionPosMiddle.size : sectionPosBottom.size;
		resourceSection.size = showResource || showRegrow ? sectionPosBottom.size : sectionPosMiddle.size;
	}
	else
	{
	...
	// Resource stats
	resourceSection.hidden = !showResource && !showRegrow;
	...
	if (showRegrow)
	{
		...
		let unitResourceBar = Engine.GetGUIObjectByName("resourceBar");
		let resourceSize = unitResourceBar.size;
		resourceSize.rright = entState.regrowing;
		
		unitResourceBar.size = resourceSize;
		Engine.GetGUIObjectByName("resourceLabel").caption = "Growing:";
		Engine.GetGUIObjectByName("resourceStats").caption = "" + entState.regrowing + "/ 100";
	}
