var g_TooltipTextFormats = {
	"unit": { "font": "sans-10", "color": "0 200 255" },
...
var g_AttackTypes = {
	...
	"Secondary": translate("Secondary Attack:"),
...
function getAttackTooltip(template)
{
	...
	{		
		if (type == "Slaughter" || type == "NeedsManPower" || type == "MountDamage")
			continue; // Slaughter is used to kill animals, so do not show it.
		
		
		if (!g_AttackTypes[type]) {
			error("Attack type " + type + " is not defined");
			continue;
		}
		...
		if (type == "Capture")
		{
			tooltips.push(sprintf(translate("%(attackLabel)s %(details)s, %(rate)s"), {
				...
				"details":
					type == "Capture" ?
						template.attack.Capture.value.toFixed(1) :
...
function getPayedTooltip(template)
{
	if (!template.payed)
		return "";
	
	return sprintf(translate("%(label)s %(resIcon)s %(count)s / %(interval)s"), {
		"label": headerFont(translate("Salary:")),
		"count": template.payed.cost,
		"resIcon": resourceIcon(template.payed.code),
		"interval": getSecondsString(template.payed.interval / 1000)
	});
}

function getConsumerTooltip(template)
{
	if (!template.consumer)
		return "";

	let resCodes = g_ResourceData.GetCodes().filter(res => !!template.consumer.rates[res]);
	if (!resCodes.length)
		return "";

	let fedString = "";
	
	if (template.consumer.fed)
		fedString = headerFont("\nFed bonus: ") + template.consumer.fed + resourceIcon(template.consumer.fedRes);
	

	return sprintf(translate("%(label)s %(details)s %(bonus)s"), {
		"label": headerFont(translate("Consuming:")),
		"details": sprintf(translate("%(resources)s / %(time)s"), {
			"resources":
				resCodes.map(
					res => sprintf(translate("%(resourceIcon)s %(rate)s"), {
						"resourceIcon": resourceIcon(res),
						"rate": template.consumer.rates[res]
					})
				).join("  "),
			"time": getSecondsString(template.consumer.interval / 1000)
		}),
		"bonus": fedString
	});
}
