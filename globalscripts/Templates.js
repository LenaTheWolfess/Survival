...
function GetTemplateDataHelper(template, player, auraTemplates, resources, damageTypes, modifiers={})
{
	...
	{
		ret.payed = {
			"interval": +template.Payed.Interval,
			"cost": +template.Payed.Sum,
			"code": template.Payed.Code
		};
	}
	...
	{
		ret.consumer = {
			"interval": +template.Consumer.Time,
			"rates": {},
			"fed": false,
			"fedRes": false
		};
		for (let g in template.Consumer.Groups) {
			let group = template.Consumer.Groups[g];
			if (!!group.Or) {
				let needs = group.Or;
				for (let n in needs) {
					let need = needs[n];
					ret.consumer.rates[need.Code] = +need.Count;
				}
			}
			...
			if (!!group.Feed) {
				ret.consumer.fed = +group.Feed.Max;
				ret.consumer.fedRes = group.Feed.Res;
			}
		}
	}
